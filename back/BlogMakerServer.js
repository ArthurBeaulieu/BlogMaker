...
global.log.info(`Starting BlogMaker server v${version}`);
const app = express();
...
db.mongoose.connect(`mongodb://${dbConfig.HOST}:${dbConfig.PORT}/${dbConfig.NAME}`, {
  ...
  utils.initSequence().then(() => {
    ...
    server.listen(3000, () => {
      global.log.info('BlogMaker server is ready to operate!');
    });
    // Add listener on kill process to properly log before exit
    process.on('SIGINT', () => {
      global.log.info('Gracefully stopping BlogMaker server');
      process.exit();
    });
  }).catch(err => {
    global.log.error(`Unable to update database model : ${err}`);
    global.log.error('Gracefully stopping BlogMaker server');
    process.exit();
  });
}).catch(err => {
